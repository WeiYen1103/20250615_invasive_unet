...
# 1 = normal / benign
# 2 = invasive
# 255 = ignore, including ROI outside and original in_situ
NUM_CLASSES = 3
PATCH_SIZE = 512
...
OUT_DIR = Path("outputs/unet_resnet34_roi_3class_no_insitu")
...
# 在 train / validation 裡轉成 0/1/2/255。
def remap_target_to_3class(targets: torch.Tensor) -> torch.Tensor:
    """
    Original target:
        0 = background
        1 = benign / normal
    ...
    Original in_situ is ignored.
    """
    new_targets = targets.clone()

    # Original in_situ -> ignore
    new_targets[targets == 2] = IGNORE_INDEX

    # Original invasive 3 -> new invasive 2
    new_targets[targets == 3] = 2
...
def format_metrics(values, name):
    class_names = ["background", "benign", "invasive"]
...
def train_one_epoch(model, loader, optimizer, criterion, scaler=None):
    ...
    for images, targets in pbar:
        ...
        # Convert 4-class target to 3-class target
        targets = remap_target_to_3class(targets)
...
def main():
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        f.write(
            ...
            "iou_background,iou_benign,iou_invasive,mean_iou,"
            "dice_background,dice_benign,dice_invasive,mean_dice,lr\n"
        )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(
                ...
                f"{val_ious[0]:.6f},{val_ious[1]:.6f},{val_ious[2]:.6f},{mean_iou:.6f},"
                f"{val_dices[0]:.6f},{val_dices[1]:.6f},{val_dices[2]:.6f},{mean_dice:.6f},"
                f"{current_lr:.8e}\n"
            )
