...
# 關掉scale/crop 
SCALE_CROP_AUG_PROB = 0.0
SCALE_CROP_RANGE = (0.92, 1.0)
...
class BreastTumorPatchDataset(Dataset):
    ...
    def _apply_augmentation(self, image: np.ndarray, mask: np.ndarray, roi: np.ndarray):
        ...
        roi: H x W
        """
        if random.random() < SCALE_CROP_AUG_PROB:
            image, mask, roi = self._apply_scale_crop_augmentation(image, mask, roi)
    ...
    def _apply_scale_crop_augmentation(
        self,
        image: np.ndarray,
        mask: np.ndarray,
        roi: np.ndarray,
        ...
        """
        Apply a mild random crop + resize to simulate small scale changes.
        Keep mask / ROI aligned with the RGB image.
        """
        ...
        roi_crop = roi[top:top + crop_h, left:left + crop_w]

        image = np.array(
            Image.fromarray(image_crop).resize((width, height), resample=Image.BILINEAR),
            dtype=np.uint8,
        )
        mask = np.array(
            Image.fromarray(mask_crop).resize((width, height), resample=Image.NEAREST),
            dtype=np.uint8,
        )
        roi = np.array(
            Image.fromarray(roi_crop).resize((width, height), resample=Image.NEAREST),
            dtype=np.uint8,
        )
